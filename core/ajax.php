...
error_reporting(E_ALL);
        ...
        if($category_id == "All" && $tag_id == "All"){
            $sql = "select portfolio.* from (SELECT portfolio.*,category.name FROM portfolio inner join category on portfolio.category_id=category.id) as portfolio LEFT JOIN portfolio_order on portfolio_order.portfolio_id = portfolio.id where portfolio_order.category_id = '0' AND portfolio_order.tag_id='ALL' order by portfolio_order.position ASC ";
