<?php
    ...
    if(isset($_GET['Action']) && $_GET['Action']=='getPortfolio')
    {
        ...
        if($category_id == "All" && $tag_id == "All"){
            ...
            $sql = "select portfolio.* from (SELECT portfolio.*,category.name FROM portfolio inner join category on portfolio.category_id=category.id) as portfolio LEFT JOIN portfolio_order on portfolio_order.portfolio_id = portfolio.id where status ='1' AND   (portfolio_order.tag_id ='ALL' || portfolio_order.tag_id IS NULL) AND (portfolio_order.category_id ='0' || portfolio_order.category_id is null)   order by portfolio_order.position ASC ";
