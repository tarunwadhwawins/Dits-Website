...
if($category_id == 0 && $tag_id == "ALL"){
    ...
    $sql = "select portfolio.* from (SELECT portfolio.*,category.name FROM portfolio inner join category on portfolio.category_id=category.id group by portfolio.id) as portfolio LEFT JOIN portfolio_order on portfolio_order.portfolio_id = portfolio.id where (portfolio_order.tag_id ='ALL' || portfolio_order.tag_id IS NULL) AND (portfolio_order.category_id ='0' || portfolio_order.category_id is null)   order by portfolio_order.position ASC; ";
